using System;
using System.Text;

namespace ParadoxReader
{
    /// <summary>
    /// C# port of pxlib's px_crypt.c. Implements the substitution-cipher
    /// used by BDE/Paradox to obfuscate password-protected .DB (and .MB)
    /// blocks, and the checksum algorithm used to turn a plain-text
    /// password into the 32-bit "encryption" key stored in the table
    /// header (encryption1/encryption2 fields).
    ///
    /// Note: this is NOT strong encryption. The 32-bit key is stored
    /// directly in the table header, so any reader (this one included)
    /// can decrypt the data without ever knowing the original password -
    /// this mirrors the real BDE behavior where "master passwords" work
    /// for any table, because the password itself only gates the BDE UI,
    /// not the actual data.
    /// </summary>
    internal static class PxCrypt
    {
        private static readonly byte[] TableA =
        {
            
        };

        private static readonly byte[] TableB =
        {
            
        };

        private static readonly byte[] TableC =
        {
            
        };

        // px_encrypt_chunk() / px_decrypt_chunk() {{{

        private static void EncryptChunk(byte[] src, int srcOffset, byte[] dst, int dstOffset,
            byte a, byte b, byte c, byte d)
        {
            var tmp = new byte[256];
            for (int x = 0; x < 256; x++)
            {
                int y = (TableC[x] - d) & 0xff;
                tmp[y] = (byte)(src[srcOffset + x]
                    ^ TableA[(x + a) & 0xff]
                    ^ TableB[(y + b) & 0xff]
                    ^ TableC[(y + c) & 0xff]);
            }
            Array.Copy(tmp, 0, dst, dstOffset, 256);
        }

        private static void DecryptChunk(byte[] src, int srcOffset, byte[] dst, int dstOffset,
            byte a, byte b, byte c, byte d)
        {
            var tmp = new byte[256];
            for (int x = 0; x < 256; x++)
            {
                int y = (TableC[x] - d) & 0xff;
                tmp[x] = (byte)(src[srcOffset + y]
                    ^ TableA[(x + a) & 0xff]
                    ^ TableB[(y + b) & 0xff]
                    ^ TableC[(y + c) & 0xff]);
            }
            Array.Copy(tmp, 0, dst, dstOffset, 256);
        }

        // }}}

        // px_encrypt_chunk2() {{{

        private static void EncryptChunk2(byte[] src, byte[] dst, int lenpassw)
        {
            var tmp = new byte[256];
            Array.Copy(src, tmp, 256);
            for (int i = lenpassw; i < 256; i++)
            {
                int x = tmp[i - lenpassw];
                int y = TableA[x] ^ i;
                tmp[i] = (byte)y;
            }
            Array.Copy(tmp, dst, 256);
        }

        // }}}

        // makeLongFromBuff() {{{

        private static long MakeLongFromBuff(long partial, byte[] buff)
        {
            long i = 0, y = 0, z = 0;
            while (i <= 255)
            {
                long x = buff[i];
                y ^= x;
                i++;
                x = buff[i];
                z ^= x;
                i++;
            }
            long result = (z << 8) | y;
            if (result == 0) result = 1;
            result = (result << 16) | partial;
            return result;
        }

        // }}}

        /// <summary>
        /// Calculates the checksum as it is stored in the header field
        /// encryption/encryption2 of the .DB file (px_passwd_checksum()).
        /// </summary>
        public static int PasswordChecksum(string password)
        {
            if (string.IsNullOrEmpty(password))
                return 0;

            var passwordBytes = Encoding.GetEncoding("ISO-8859-1").GetBytes(password);
            int len = passwordBytes.Length;

            var buff = new byte[256];
            int count = 256;
            int destOffset = 0;
            while (count >= len)
            {
                Array.Copy(passwordBytes, 0, buff, destOffset, len);
                destOffset += len;
                count -= len;
            }
            if (count > 0)
            {
                Array.Copy(passwordBytes, 0, buff, destOffset, count);
            }

            EncryptChunk(buff, 0, buff, 0, buff[0], buff[1], buff[2], buff[3]);
            long x = buff[0];
            long y = buff[1];
            long partial = (y << 8) + x;

            // Only the first `len` bytes are overwritten with the plain
            // password; the remainder intentionally keeps the leftover
            // encrypted bytes from above, matching pxlib's behavior.
            Array.Copy(passwordBytes, 0, buff, 0, len);
            EncryptChunk2(buff, buff, len);
            EncryptChunk(buff, 0, buff, 0, buff[0], buff[20], buff[40], buff[255]);

            long result = MakeLongFromBuff(partial, buff);
            return unchecked((int)(result & 0xFFFFFFFFL));
        }

        /// <summary>
        /// Encrypts/decrypts a whole .DB block in-place (px_encrypt_db_block /
        /// px_decrypt_db_block). blockSize must be a multiple of 256.
        /// blockNumber is the 1-based block number as used on-disk by pxlib.
        /// </summary>
        public static void EncryptDbBlock(byte[] buffer, int offset, int blockSize, uint encryption, uint blockNumber)
        {
            byte a = (byte)(encryption & 0xff);
            byte b = (byte)((encryption >> 8) & 0xff);
            int chunkCount = blockSize / 256;
            for (int chunk = 0; chunk < chunkCount; chunk++)
            {
                int chunkOffset = offset + (chunk << 8);
                EncryptChunk(buffer, chunkOffset, buffer, chunkOffset, a, b, (byte)chunk, (byte)blockNumber);
            }
        }

        public static void DecryptDbBlock(byte[] buffer, int offset, int blockSize, uint encryption, uint blockNumber)
        {
            byte a = (byte)(encryption & 0xff);
            byte b = (byte)((encryption >> 8) & 0xff);
            int chunkCount = blockSize / 256;
            for (int chunk = 0; chunk < chunkCount; chunk++)
            {
                int chunkOffset = offset + (chunk << 8);
                DecryptChunk(buffer, chunkOffset, buffer, chunkOffset, a, b, (byte)chunk, (byte)blockNumber);
            }
        }
    }
}
